...
locals {
  valid_locales          = jsondecode(file("reference/locales.json"))
  valid_keyboard_layouts = jsondecode(file("reference/keyboard_layouts.json"))
  valid_timezones = distinct(concat(
    flatten([for tz in jsondecode(file("reference/timezones.json")) : tz.utc]),
    ["UTC", "GMT"]
  ))
  locale_index          = index(local.valid_locales, var.locale)
  keyboard_layout_index = index(local.valid_keyboard_layouts, var.keyboard_layout)
  timezone_index        = index(local.valid_timezones, var.timezone)
  boot_command_efi = [
    "c<wait>",
  ...
  disk_adapter_type    = var.arch == "arm64" ? "nvme" : var.vm_disk_adapter_type
  ...
  iso_target_path = "${local.repo_root}/iso/"
  iso_urls = [
