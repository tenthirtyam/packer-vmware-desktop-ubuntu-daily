...
# Prepend known VMware desktop hypervisor binary directories to PATH so 'vmware-vmx' can be found
...
# Resolve the Ubuntu Server daily-live base URL for the current release layout.
resolve_iso_base_url() {
	local index_url="https://cdimage.ubuntu.com/ubuntu-server/"
	local legacy_url="${index_url}daily-live/current"
	local release url

	if [[ -n "${ISO_BASE_URL:-}" ]]; then
		printf '%s\n' "${ISO_BASE_URL}"
		return 0
	fi

	if curl -sf "${legacy_url}/SHA256SUMS" >/dev/null 2>&1; then
		printf '%s\n' "${legacy_url}"
		return 0
	fi
	...
	while IFS= read -r release; do
		[[ -z ${release} ]] && continue
		url="${index_url}${release}/daily-live/current"
		if curl -sf "${url}/SHA256SUMS" >/dev/null 2>&1; then
			printf '%s\n' "${url}"
			return 0
		fi
	done < <(curl -sf "${index_url}" | grep -oE 'href="[a-z]+/"' | sed 's/href="//;s/\/"$//' | sort -r)

	printf "\033[31m[ERROR]\033[0m Unable to find an Ubuntu Server daily-live repository.\n" >&2
	return 1
}
...
show_help() {
	...
	printf "\nEnvironment:\n"
	printf "  ISO_BASE_URL       Override Ubuntu daily-live base URL\n"
	printf "\nDirectories:\n"
	printf "  ISO files:     %s/iso/ (created automatically)\n" "${REPO_ROOT}"
	printf "  VM output:     %s/output/ (created automatically)\n" "${REPO_ROOT}"
...
# Exit immediately when help is requested.
exit_on_help_request() {
	for arg in "$@"; do
		case "${arg}" in
		--help | -h)
			show_help
...
# Resolve ISO file name, URL, and checksum from the Ubuntu daily-live repository.
resolve_iso_metadata() {
	ISO_BASE_URL=$(resolve_iso_base_url) || exit 1

	local iso_name_file checksums_file
	iso_name_file=$(mktemp)
	checksums_file=$(mktemp)

	{
		if iso_name=$(curl -sf "${ISO_BASE_URL}/" | grep -o "href=\"[^\"]*live-server-${GUEST_ARCH}.iso\"" | sed 's/href="//; s/"$//' | head -n 1) && [[ -n ${iso_name} ]]; then
	...
	{
		if checksums=$(curl -sf "${ISO_BASE_URL}/SHA256SUMS"); then
	...
	rm -f "${iso_name_file}" "${checksums_file}"
}
...
initialize() {
	...
	if init_output=$(cd "${REPO_ROOT}" && packer init ./src 2>&1); then
...
validate() {
	...
	if validate_output=$(cd "${REPO_ROOT}" && packer validate "${PACKER_VARS[@]}" ./src 2>&1); then
...
build() {
	...
	cd "${REPO_ROOT}" && packer build --force "${PACKER_VARS[@]}" ./src
}
...
main() {
	...
	if [[ -f "${SCRIPT_DIR}/overrides.pkrvars.hcl" ]]; then
		PACKER_VARS+=("-var-file" "${SCRIPT_DIR}/overrides.pkrvars.hcl")
		printf "\e[33m  => Using overrides from src/overrides.pkrvars.hcl\e[0m\n"
	fi
...
DEFAULT_DOWNLOAD_DIR="${REPO_ROOT}/iso"
